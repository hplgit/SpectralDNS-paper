...
For straight-forward comparison, the Python solver with slab decomposition has been used as prototype for a pure C++ implementation (https://github.com/mikaem/ spectralDNS/blob/ master/cpp/spectralDNS.cpp). The only significant difference is that for MPI communication the routines provided by FFTW have been used directly. Neglecting the MPI implementation and running only in serial for three meshes of size $(32^3, 64^3, 128^3)$, the C++ code runs at (0.0092, 0.083, 0.80) seconds per time step respectively on a MacBook Pro with 2.8 GHz Intel Core i7. In comparison, the pure Python solver runs at (0.013, 0.12, 1.3) seconds and we conclude that the C++ code is approximately 30-40 \% faster. Considering the issue recently discussed with the cross product, we are all in all quite happy with the performance of the Python solver.  And, fortunately, several strategies are possible to boost performance. 

We have considered four different strategies for speeding up our code: \inpyth{numba}, \inpyth{numexpr}, \inpyth{weave} and \inpyth{Cython}, where the best results have been obtained for \inpyth{numba} and \inpyth{Cython}.  The \inpyth{numba} module uses just in time compilation of functions decorated with a \inpyth{@jit} signature. For efficient speed-up the code must be rewritten using explicit for loops. The advantage is that no compilation or installation is required by the user, the decorator takes care of it all. The downside is that \inpyth{numba} is difficult to install on non-standard platforms like the Blue Gene/P. A numba optimized cross product routine may be implemented in module \inpyth{numba_module.py} and code will be generated and compiled when the module is imported.
...
To verify weak scaling we continue to use the Taylor Green test case and keep the ratio of mesh size to number of processors ($N^3/M$) constant, which ideally should lead to CPU times proportional to $\log_2 N$. Both slab and pencil decompositions are tested using four meshes of size $N=(2^7, 2^8, 2^9, 2^{10})$ and single precision arithmetics. For these meshes the slab decomposition uses $M=(2, 16, 128, 1024)$ number of CPUs and the pencil uses $M=(4, 32, 256, 2048)$ with $P1=(2, 4, 16, 32)$. Note that for $N=2^{10}(=1024)$ the maximum number of CPUs for the slab decomposition is reached, whereas the pencil decomposition may still use many more. We measure the real computing time per time step and plot the results in Figure \ref{fig:weak_scaling_shaheen} scaled by $N^3 \log_2N$. Slab and pencil results are marked by squares and dots, respectively, and the solid line corresponds to ideal scaling. Both methods show excellent scaling, and only start to trail off the ideal curve for the largest mesh. The slab decomposition is slightly more efficient than pencil, which is in agreement with previous studies (need ref).

An obvious scaling problem for the pure numpy implementation is the transpose operation used in the FFTs (see implementation of \inpyth{fftn_mpi}), where a for loop is used over all processes (or over all processes in a group for pencil decomposition) to rearrange data. The for loops are inefficient in Python and should be avoided. For relatively small numbers of processors the cost of this for loop is negligible, but when reaching thousands of processors, the loop becomes too slow leading to poor scaling. When the for loop is moved to Cython, better weak scaling is observed also for highest number of cores shown in Figure \ref{fig:weak_scaling_shaheen}.


\begin{figure}
\includegraphics[scale=1]{figs/weak_scaling_shaheen.png}
\caption{Weak scaling of pure numpy/mpi4py/pyfftw DNS solver.}
...
\begin{figure}
\includegraphics[scale=1]{figs/strong_scaling_shaheen_pencil_512.png}
\caption{Strong scaling of pure numpy/mpi4py/pyfftw DNS solver.}
