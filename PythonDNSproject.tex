...
\author{Mikael Mortensen\thanks{Department of Mathematics, University of Oslo, and Center for Biomedical Computing, Simula Research Laboratory. Email: \texttt{mikael.mortensen@gmail.com}.}, Hans Petter Langtangen\thanks{Center for Biomedical Computing, Simula Research Laboratory, and Department of Informatics, University of Oslo. Email: \texttt{hpl@simula.no}.} and David Ketcheson\thanks{Department of Applied Mathematics and Computational Science, King Abdullah University of Science and Technology, Saudi-Arabia. Email: \texttt{David.Ketcheson@kaust.edu.sa}.}}
%\date{}							% Activate to display a given date or no date

\begin{document}
\maketitle

\hpl{I think the pygments coloring of inline Python code (inpyth) looks strange in the text. I suggest switching (redefining inpyth) to plain black verbatim.}
...
\hpl{A comment on why such explicit integrators work well? Stability is typically governed by $\nu\Delta t < 1$ or so for Forward Euler, but I guess the time step for DNS has to be so small that stability even for Forward Euler is no issue?}
...
large temporary arrays (which often arises from \texttt{numpy} code with basic array arithmetics). \hpl{There is no used of in-place arithmetics or operations in the above code. The point is first illustrated in \texttt{Cross} and \texttt{Curl} and \texttt{ComputeRHS}.}
...
\hpl{It seems that the FFI functions just take input and returns a new array (?). In that case, the \texttt{u} object sent to the Python functions gets lost, and the FFT functions creates a new object references by \texttt{u}. The FFT functions would need to get the result array as argument.}
...
\hpl{All the details with \texttt{kmax\_dealias} and \texttt{dealias} were disturbing when explaining the very simple algorithm of the entire solver...so I a moved this part after the main loop together with the explaining paragraph.}
...
are scaled by $N^3 \log_2N$ and plotted in Figureure \ref{fig:weak_scaling_shaheen}. Slab and pencil results
are marked by squares and dots, respectively, the C++ results are shown as diamonds and the solid line
corresponds to ideal scaling. From Figure~\ref{fig:weak_scaling_shaheen} it is evident that both slab and pencil versions of the Python solver show excellent scaling,
...
Figure.~\ref{fig:weak_scaling_shaheen} if they were equally efficient. The slab decomposition is thus seen to
...
\caption{Weak scaling of pure \texttt{numpy/mpi4py/pyfftw} DNS solver. The slab (squares) decomposition used $4 \cdot 64^3$ nodes per core, whereas the pencil decomposition (dots) used $2 \cdot 64^3 $. The C++ solver uses slab decomposition and MPI communication is performed through the FFTW library. }
...
particular case at least, is actually faster than the parallel implementation provided through the FFTW library.

We note finally that an obvious scaling problem for the pure \texttt{numpy} implementation is the transpose operation used in the implementation of the FFTs
...
but when approaching thousands of processors, the loop becomes too slow leading to poor scaling. This for loop is the main reason why the \texttt{numpy} solver performs rather poorly for the largest number of cores seen for the slab solver in Figure~\ref{fig:strong_scaling_shaheen}.
