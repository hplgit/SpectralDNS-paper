...
\begin{equation}
\bm{x} = \left\{(x_i, x_j, x_k) = \left( \frac{2\pi i}{N}, \frac{2\pi j}{N}, \frac{2\pi k}{N} \right): \, i,j,k \in 0,\ldots, N-1 \right\}.
\label{eq:realmesh}
\end{equation}
...
\begin{align}
\mathcal{F}_{k_z}(\bm{u}) = \hat{\bm{u}}_{k_z}(t) &= \frac{2\pi}{N}\sum_{k=0}^{N-1}{\bm{u}({x}_k, t)}e^{-\imath k_z x_k}, \quad k_z=-N/2+1, \ldots, N/2,
\end{align}
with the inverse transform
\begin{align}
\mathcal{F}^{-1}_{z} (\hat{\bm{u}}) =\bm{u}(x_k, t) &= \frac{1}{2\pi}\sum_{k_z=-N/2+1}^{N/2}\hat{\bm{u}}_{k_z}(t)e^{\imath k_z {x}_k}, \quad k=0, \ldots, N-1.
\end{align}
...
By taking the three-dimensional Fourier transform of the Navier-Stokes equations and subsequently the analythical spatial derivatives in spectral space, we obtain a system of ordinary differential equation for $\hat{\bm{u}}_{\bm{k}}$, and the continuity equation reduces to an orthogonal inner product:
\begin{align}
...
A few solutions to the loading problem are known. The \inpyth{MPI_Import} Python module of Langton \cite{mpi_import} is designed to overload the built in import mechanism of Python such that only one rank (the zero rank) searches the filesystem for a module and then subsequently broadcasts the results to the remaining ranks. A drawback is the Python implementation where several modules \inpyth{(sys, imp, __builtin__,types, mpi4py)} need to be imported through the regular Python import mechanism. A second approach of Langton is to rewrite the regular finders and loaders modules such that one rank do all the initial work, caching the modules it finds. 
...
An obvious scaling problem for the pure numpy implementation is the transpose operation used in the FFTs 
(see implementation of \inpyth{fftn_mp i}), where a for loop is used over all processes (or over all 
...
but when reaching thousands of processors, the loop becomes too slow leading to poor scaling. When the for 
loop is moved to Cython, better weak scaling is observed also for highest number of cores shown in Figure 
\ref{fig:weak_scaling_shaheen}. Results for the optimized Cython version of the code is not shown, but they are found to fall almost on top of the C++ diamonds.

\begin{figure}
\includegraphics[scale=1]{figs/weak_scaling_shaheen_numpy.png}
\caption{Weak scaling of pure numpy/mpi4py/pyfftw DNS solver. The slab (squares) decomposition used $4 \cdot 64^3$ nodes per core, whereas the pencil decomposition (dots) used $2 \cdot 64^3 $. The C++ solver uses slab decomposition. The solid line represents ideal scaling.}
\label{fig:weak_scaling_shaheen}
\end{figure}

\subsubsection{Strong scaling}
Strong scaling over a wide range of processors is more challenging than weak.
