...
\title{High performance Python simulations of turbulent flows}
...
A very important part of the implementation is the mesh decomposition (we have used both slab and pencil) and 3D parallel Fast Fourier Transforms (FFT). The mesh decomposition and FFT routines have been implemented in Python using serial FFT routines (either \inpyth{numpy} or \inpyth{pyfftw}), \inpyth{numpy} array manipulations and with MPI communications handled by \inpyth{mpi4py}. We show how we can write a 3D parallel FFT routine in Python for a slab mesh decomposition in 4 lines of compact Python code, for which the parallel performance on Shaheen is found to be slightly better than similar routines provided through the FFTW library. For a pencil mesh decomposition 7 lines of code is required to actually perform the transformation.
...
\cite{turbo}. However, the user interfaces to these codes are not sopthisticated and user-friendly,
...
the nature of codes in low-level languages.
...
\end{python}
Obviously, more advanced and accurate integrators may be easily added by modifying the final \inpyth{while} loop. See the Appendix [\ref{sec:appendix}] for a fourth order Runge Kutta implementation.
...
The entire 3D parallel FFT may be implemented with preallocation of a work array to do MPI and 4 lines of Python code:
...
To verify weak scaling we continue to use the Taylor Green test case and keep the ratio of mesh size to number of processors ($N^3/M$) constant, which ideally should lead to CPU times proportional to $\log_2 N$. The solver used in this section is not optimized in any way beyond \texttt{numpy/mpi4py/pyfftw}.
...
are scaled by $N^3 \log_2N$ and plotted in Figure \ref{fig:Weak_scaling} alongside the C++ implementation. Figure \ref{fig:weak_scaling_shaheen} and \ref{fig:weak_scaling_shaheen_opt} shows, respectively, the weak scaling results for the non-optimized pure Python solver and the solver optimized through Cython. From Figure \ref{fig:weak_scaling_shaheen} it is evident that both slab and pencil versions of the Python solver show excellent scaling,
and only start to trail slightly off the ideal curve for the largest mesh sizes. The C++ solver is seen to be about $10-20 \%$ faster than the pure
Python solver for the current range, and evidently the difference diminishes with increasing number of cores. In other words, the MPI implementation of 3D FFT for the pure Python solver scales better than the 3D FFT routines provided by FFTW.  Note that the pencil computations use twice as many cores for the same mesh size as the slab computations. As such the two methods would fall on the same line in
Figure.~\ref{fig:weak_scaling_shaheen} if they were equally efficient. The slab decomposition is thus seen to be more efficient than pencil, which is generally in agreement with previous studies (see, e.g., \cite{pekurovsky2012}).

Figure \ref{fig:weak_scaling_shaheen_opt} shows the performance of the Python solvers optimized with Cython. Now there is apparently very little separating the C++ and the Python solvers, and the Python solver actually performs slightly better. Both the slab and pencil optimized implementations are seen to be approximately $20 \% $ faster than their pure Python counterparts.

\begin{figure}
\subfigure[]{
\includegraphics[scale=0.8]{figs/weak_scaling_shaheen_numpy_noopt.png}
\label{fig:weak_scaling_shaheen}
}
\subfigure[]{
\includegraphics[scale=0.8]{figs/weak_scaling_shaheen_numpy_opt.png}
\label{fig:weak_scaling_shaheen_opt}
}
\caption{Weak scaling of DNS solver. The slab (squares) decomposition used $4 \cdot 64^3$ nodes per core, whereas the pencil decomposition (dots) used $2 \cdot 64^3 $. The C++ solver uses slab decomposition with MPI communication performed through the FFTW library. The Python solver in \subref{fig:weak_scaling_shaheen} is not optimized, whereas the solver in \subref{fig:weak_scaling_shaheen_opt} has been optimized through Cython.}
...
Python/Cython code for the Taylor Green test case with a computational box of size $512^3$. In Figure
\ref{fig:Strong_scaling} we plot for both solvers the computing time per time step, for both the pencil and slab decompositions. The number of
...
pencil decomposition. The scaling is, quite frankly, excellent for both the optimized and non-optimized versions, and good speedup is observed
throughout the entire range. Interestingly, the optimized version of the code is seen
to more than match the performance of the C++ solver for this particular
case. In this regard it is interesting to note that the C++ and the Python code both use the same compiled FFTW library. However, the Python code only makes use of the serial versions of the FFTW
routines and performs the MPI communications itself through
\inpyth{mpi4py}, whereas the C++ code uses the built in MPI communication routines of FFTW. From Figure~\ref{fig:strong_scaling_shaheen} it is evident
that the optimized Python solver performes
gradually better than the C++ solver for a larger number of cores. This enhanced performance
means that our Python implementation of the parallel 3D FFT routine, for this
particular case at least, is scaling better than the parallel implementation provided through the FFTW library. Note that Cython can do nothing to enhance the performance of the 3D \inpyth{fftn_mpi} routine, and the superior scaling of the Python slab solver is as such evidenced also in Figure \ref{fig:strong_scaling_shaheen}.

\begin{figure}
\subfigure[]{
\includegraphics[scale=0.8]{figs/strong_scaling_shaheen_noopt_512.png}
\label{fig:strong_scaling_shaheen}
}
\subfigure[]{
\includegraphics[scale=0.8]{figs/strong_scaling_shaheen_opt_512.png}
\label{fig:strong_scaling_shaheen_opt}
}
\caption{Strong scaling of the  DNS solver. The Python solver in \subref{fig:strong_scaling_shaheen} is not optimized, whereas the solver in \subref{fig:strong_scaling_shaheen_opt} has been optimized through Cython.}
